...
8. Скопировать файл .env.local.dist (в этом файле указанны все переменные окружения, которые нужны для корректной работы автотестов, но значения для них не указаны)
    - Вставить файл в корень проекта (там же, где расположен .env.local.dist) переименовав его в .env.local - в этом файле мы указываем все значения переменных окружения
