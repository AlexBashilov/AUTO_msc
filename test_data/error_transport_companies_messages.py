...
class ErrorTransportCompaniesMessages(enum.StrEnum):
    ...
    CONTRACT_DATE_BLANK = (
        'parsing body body from "" failed, because parsing time "" as "2006-01-02": cannot parse "" '
        'as "2006"'
    )
    CONTRACT_NUMBER_BLANK = "Ошибка валидации запроса: ContractNumber: cannot be blank."
    CONTACT_FACE_TRANSPORTER_BLANK = (
        "Ошибка валидации запроса: FullName: cannot be blank."
    )
    POSITION_OF_CONTACT_PERSON_BLANK = (
        "Ошибка валидации запроса: Position: cannot be blank."
    )
    ...
    REGISTERED_ADDRESS_TRANSPORTER_BLANK = (
        "body.params.legalAddress in body is required"
    )
