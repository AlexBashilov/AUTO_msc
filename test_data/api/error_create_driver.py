...
class CreateDriverError(enum.StrEnum):
    ...
    PASSPORT_FULL_NUMBER_REQUIRED_ERROR = (
        "body.params.passportFullNumber in body is required"
    )
    LICENSE_NUMBER_REQUIRED_ERROR = "body.params.licenseNumber in body is required"
    EMPTY_TC_ID_ERROR = (
        "Ошибка валидации запроса: массив TransportCompanyIDs не должен быть пустым"
    )
    ...
    EMPTY_PHONE_NUMBER_ERROR = (
        "Ошибка валидации запроса: поле PhoneNumber должно быть не пустое"
    )
    EMPTY_PASSPORT_NUMBER_ERROR = (
        "Ошибка валидации запроса: поле PassportFullNumber должно быть не пустое"
    )
    EMPTY_LICENSE_NUMBER_ERROR = 'Ошибка создания водителя: Необходимо заполнить поле "Водительское удостоверение"'
