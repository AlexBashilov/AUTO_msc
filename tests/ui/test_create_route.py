...
class TestCreateRoute:
    ...
    def test_create_invalid_routes(self, db_connection, driver, example):
        ...
        route.create_route()
        for point_operation in example['routePointOperation']:
            route.add_operation_in_route(point_operation['pointName'], point_operation['operationType'],
    ...
    def test_create_exist_route(self, db_connection, driver):
        ...
        route.add_operation_in_route(route_points[1], RouteOperationType.UNLOADING, route_points[0])
        ...
        route.check_created_route(user_credentials['login'], route_name, federal_distinct)
        route.create_route()
        route.add_operation_in_route(route_points[0], RouteOperationType.LOADING)
        route.add_operation_in_route(route_points[1], RouteOperationType.UNLOADING, route_points[0])
        route.set_federal_district(federal_distinct)
    ...
    def test_create_route_based_on(self, db_connection, driver):
        ...
        route.save_route()
        route.check_created_route(user_credentials['login'], first_route_name, federal_distinct)
        route.create_route_based_on()
        route.check_open_route(route_point_operations)
        route.delete_operation_in_route_by_number(2)
        route.add_operation_in_route(ShopList.MSK_KUBINKA_SHOP, RouteOperationType.UNLOADING, ShopList.MSK_DOMODEDOVO_WAREHOUSE)
        route.save_route()
        ...
        route.check_created_route(user_credentials['login'], second_route_name, federal_distinct)
        route.delete_first_route()
