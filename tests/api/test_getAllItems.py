...
class TestGetAllItems:
    ...
    #     api_utils.assert_schema(response, getAllItems.GetAllItemsResponseSchema) #Тест написан
        # Сходить в БД и сделать селект всех строк - сделано
    ...
    def test_get_items_by_id(self,client, db_connection):
        ...
        print(response_by_one)

        api_utils.assert_response_code(HTTPStatus.OK, response_by_one.status_code)
        api_utils.assert_schema(response_by_one, GetOneItemsByIDResponseSchema )
        # print(expected_response)
        # print(insert_item)
